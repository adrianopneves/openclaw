...
export function formatQueuePosts(posts) {
  ...
  posts.forEach((post, index) => {
    ...
    const services = post.channelService || 'unknown';
    const scheduled = post.dueAt ? new Date(post.dueAt).toISOString() : 'n/a';
  ...
  program
    ...
    .action(async (options) => {
      ...
      try {
        const activeApi =
          api || new BufferApi({ ...getConfig(), apiKey: validateApiKey(getConfig().apiKey) });
        const posts = await activeApi.getScheduledPosts('69ad8eb4d1da09ec8352c8c7');
